...
df['SMA' + str(window3)] = df['close'].rolling(window3).mean()
...
df['SMA' + str(window3)] = df['SMA' + str(window3)].apply(lambda x: round(x, 2))

# Generate signals
df['signal'] = np.where((df['SMA' + str(window1)] == df['SMA' + str(window2)]) & (df['SMA' + str(window1)] == df['SMA' + str(window3)]), np.nan, np.nan)
df['signal'] = np.where((df['SMA' + str(window1)] > df['SMA' + str(window2)]) & (df['SMA' + str(window1)] > df['SMA' + str(window3)]), 1.0, 0.0)
...
first_value_index = df['SMA' + str(window3)].first_valid_index()
...
df.to_csv("Triple_SMA.csv", index=False)
